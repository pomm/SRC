...
void Gen_gn_pimp_up(int N = 100000, int A = 2, bool printOutput = false){
  ...
  std::string line;
  const int lines =46;
  TH1F* GammaBeamHist= new TH1F("GammaBeamHist","", lines, 2.8, 12.);
  int i = 0;
  ...
  */
  ...
  for (int i=0; i<N; i++){
    ...
    for(int k=0; k<=Nthpo; k++){
      ...
      //      cout<<"thetaP4 = "<<theta_P4<<", absP4 = "<<absP4<<endl;
      ...
      //    Target       30 days   nb->b   b->cm2
      con = 1.27e24  * 3600*24*30 * 1e-9 * 1e-24; // target 2
